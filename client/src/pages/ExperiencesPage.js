...
export default function ExperiencesPage({ isInCollection, toggleCollection }) {
  ...
  const filtered = useMemo(() => {
    let results = filter === 'All' ? experiences : experiences.filter(e => e.type === filter);
    switch (sort) {
  ...
  return (
    <>
      <PageHero
        eyebrow="Module 03 — The Experience Layer"
        title={<>Feel the <em>thunder.</em></>}
        subtitle="Helicopter flights, safaris, white-water rapids, moonlit canoes — the activities that make Victoria Falls unforgettable."
      ...
      <Container>
        ...
        <Count>{filtered.length} {filtered.length === 1 ? 'experience' : 'experiences'} ready to book</Count>

        {filtered.length === 0 ? (
          <Empty><h3>No experiences found</h3><p>Try another category.</p></Empty>
        ) : (
          <motion.div initial="hidden" animate="visible" variants={stagger}>
            <Grid>
              {filtered.map(exp => (
                <Card key={exp.id} variants={fadeUp}>
                  <Heart
                    $saved={isInCollection(exp.id)}
                    onClick={e => { e.preventDefault(); toggleCollection({ id: exp.id, type: 'experience', name: exp.name, image: exp.images[0], price: exp.priceFrom }); }}
                    aria-label={isInCollection(exp.id) ? 'Remove from collection' : 'Save to collection'}
                  >
                  ...
                  <Link to={`/experiences/${exp.slug}`}>
                    <Media>
                      ...
                      <span className="diff">{exp.difficulty}</span>
                    </Media>
                    <Body>
                      <h3>{exp.name}</h3>
                      <p className="desc">{exp.description}</p>
                      <div className="foot">
                        <span className="meta">{exp.duration} · ★ {exp.rating}</span>
                        <span className="price">${exp.priceFrom} <span>/ person</span></span>
                      </div>
                    </Body>
                  </Link>
                </Card>
